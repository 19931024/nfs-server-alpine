...
while true; do
  ...
  while [ -z "$pid" ]; do
    ...
    /usr/sbin/rpc.nfsd --debug 8 --no-udp --no-nfs-version 2 --no-nfs-version 3 --nfs-version 4 
    ...
    /usr/sbin/rpc.mountd --debug all --no-udp --no-nfs-version 2 --no-nfs-version 3  --nfs-version 4
